...
require('vendor/vendor/autoload.php');
   ...
   //	$sub_sql=" and party.id=$party_id ";
            ...
            $res=mysqli_query($con,"select student_data.*, date_format(adm_date,'%d-%m-%Y') as admdt from student_data where $sub_sql order by id");
            ...
            if(mysqli_num_rows($res1)>0){
                         ...
                         while($row1=mysqli_fetch_assoc($res1)){
                           // $imagePath="" .$row1['img']. "";
                            ...
                            $html2.="<h5 style='text-align:left'>Student Admission-Fee Report-" .$row1['sese']."</h5>";
            ...
            if(mysqli_num_rows($res)>0){
                $html='<table border="1" width="100%" cellpading="8" cellspacing="0">';
                        $html.='<thead><tr><th>Sn.</th><th>CCode</th><th>CName</th><th>CFee</th><th>AdmisId.</th><th>RegNo.</th><th>RegDate</th><th>StName</th><th>FtName</th><th>MtName</th></tr></thead>';
                        $count=0;
                         while($row=mysqli_fetch_assoc($res)){
                             //$final_price=$final_price+$row['price'];
                $html.='<tr><td>'.++$count.'</td><td>'.$row['cl_cd'].'</td><td>'.$row['cl_name'].'</td><td>'.$row['st_fee'].'</td><td>'.$row['id'].'</td><td>'.$row['st_adm_no'].'</td><td>'.$row['admdt'].'</td><td>'.$row['st_name'].'</td><td>'.$row['st_fath'].'</td><td>'.$row['st_moth'].'</td>></tr>';
                }
